...
func (h *ProblemHandler) UpdateCheckInfo(w http.ResponseWriter, r *http.Request) {
	// 認証トークンを取得
	token := r.Header.Get("Authorization")
	if token == "" {
		utils.WriteErrorResponse(w, http.StatusUnauthorized, "認証トークンが必要です")
	...
	var req models.UpdateCheckInfoRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		utils.WriteErrorResponse(w, http.StatusBadRequest, "Invalid JSON")
		return
	}
	...
	if req.CheckInfo == nil {
		utils.WriteErrorResponse(w, http.StatusBadRequest, "チェック情報は必須です")
		return
	}

	// チェック情報を更新
	updatedProblem, err := h.problemService.UpdateCheckInfo(r.Context(), req, user.ID)
	if err != nil {
		if err.Error() == "problem not found or access denied" {
			utils.WriteErrorResponse(w, http.StatusForbidden, "問題が見つからないか、アクセス権限がありません")
			return
	...
	response := models.UpdateCheckInfoResponse{
		Success: true,
		Problem: updatedProblem,
	}
